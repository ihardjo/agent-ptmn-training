# chat-completions-serving Specification

## Purpose

Defines how the agent is served over HTTP: an OpenAI-compatible `POST /v1/chat/completions` endpoint backed by a plain FastAPI app, streaming and non-streaming, with tool activity surfaced in the stream and a health check for the platform. Serving depends on no MLflow `AgentServer` machinery, so any OpenAI client can talk to the agent unmodified.
