...
|---|---|---|---|---|---|
| `computing-target-adherence` | Adherence and breach counts, joining the table to wiki-held targets | ihardjo | 1 | `/wiki/notes/policies/resolution-targets.md`, `/wiki/notes/policies/breach-counting.md`, `/wiki/notes/definitions/duration-basis.md`; columns `cycle_time_hours`, `ticket_type`, `priority`, `closed_at`, `started_at` | not yet evaluated — task 5.4 |
...
| `escalating-breaches` | Escalation routing for breached tickets, with its trust state | ihardjo | 1 | `/wiki/notes/policies/escalation-matrix.md` (expired); `computing-target-adherence` | not yet evaluated — task 5.4 |
