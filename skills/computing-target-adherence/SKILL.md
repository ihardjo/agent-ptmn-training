...
Read `/wiki/openwiki/policies/resolution-targets.md`. It holds a target per
`ticket_type` and `priority`. Read the counting rules from
`/wiki/openwiki/policies/breach-counting.md`.
...
`/wiki/openwiki/definitions/duration-basis.md`. Confirm it there rather than
...
>
> - Target from `/wiki/openwiki/policies/resolution-targets.md` («state whether verified and current»).
> - Counting rules from `/wiki/openwiki/policies/breach-counting.md` (draft, unratified).
